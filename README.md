...
You might ask why we do an artist search without also including the recording information? This is because I wasn't sure about the behavior of `search_recordings` run in `strict=True` mode when we don't know the artist ID in particular, only have a guess at the approximate artist name (e.g. we say `Animals` or `Tom Petty` but want recordings under `The Animals` or `Tom Petty and the Heartbreakers`). There might be some *quite rare, but very real* situations where the search returns songs from artists we don't want (can't find any example right now, but may have come across them during testing). So, for now, it may be better to have the user explicitly confirm the artist using disambiguation information. Though needs more testing -- if I can't find any examples, may just forget it the user confirming the artist.
